...
class Model:
    ...
    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
        ...
        self.decoderType = DecoderType.WordBeamSearch
    ...
    def setupCNN(self):
        ...
        for i in range(numLayers):
            ...
            kernel2 = tf.Variable(tf.random.truncated_normal([kernelVals[i], kernelVals[i], featureVals[i+1], featureVals[i + 1]], stddev=0.1))
            conv2 = tf.nn.conv2d(input=relu, filters=kernel2, padding='SAME',  strides=(1,1,1,1))
            conv_norm2 = tf.compat.v1.layers.batch_normalization(conv2, training=self.is_train)
    ...
    def setupCTC(self):
        ...
        # word beam search decoding
            ...
            wordChars = open(WORD_CHARLIST).read().splitlines()[0]
    ...
    def save(self):
        "save model to file"
        self.snapID += 1
        self.saver.save(self.sess, SNAPSHOT_PATH, global_step=self.snapID)
